"""
XGB GridSearchCV on June-Aug 2024 training data — ALL 8 FEATURES.

The 5-fold TimeSeriesSplit runs ONLY on June-Aug training data.
...
Features (8):
  ...
  roll_vol_500, time_of_day, exchange_id, day_of_week

Scoring: neg_mean_absolute_error
GridSearchCV objective: reg:squarederror

Output:
  - data/gridsearch_details_8feat.csv
  - aapl_gridsearch_detail_8feat.png
"""
...
print("  ** September data is NOT used in the 5-fold CV — holdout only **")
...
# ── Save full table ─────────────────────────────────────────────────────────
results.to_csv("data/gridsearch_details_8feat.csv", index=False)
print(f"Saved full table -> data/gridsearch_details_8feat.csv  ({len(results):,} rows)\n")
...
print(f"\n{'=' * 120}")
print("  HOLDOUT COMPARISON: best hyperparams with both objectives (8 features)")
print(f"  Params: {best}")
print("=" * 120)
...
ax4.set_title("Feature importances\n(reg:squarederror, best params, 8 features)",
              fontsize=11, fontweight="bold")
...
text = (
    f"HOLDOUT RESULTS (Sep 2024, 8 features)\n"
    f"{'─' * 44}\n\n"
...
fig.suptitle(
    "AAPL lit buy blocks — XGB GridSearchCV detail  (8 features, search obj: squarederror)\n"
    f"{n_combos:,} combos x 3-fold TimeSeriesSplit (Jun-Aug only)  |  "
    f"Best inner-CV MAE: {-gs.best_score_:.4f} bps",
    fontsize=13, fontweight="bold", y=1.02,
)

plt.savefig("aapl_gridsearch_detail_8feat.png", dpi=150, bbox_inches="tight")
print(f"\nSaved -> aapl_gridsearch_detail_8feat.png")
