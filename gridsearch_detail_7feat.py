"""
XGB GridSearchCV on June-Aug 2024 training data — 7 FEATURES (no day_of_week).
...
Features (7):
  ...
  roll_vol_500, time_of_day, exchange_id

Scoring: neg_mean_absolute_error
GridSearchCV objective: reg:squarederror

Output:
  - data/gridsearch_details_7feat.csv
  - aapl_gridsearch_detail_7feat.png
"""
...
# ── Save full table ─────────────────────────────────────────────────────────
results.to_csv("data/gridsearch_details_7feat.csv", index=False)
print(f"Saved full table -> data/gridsearch_details_7feat.csv  ({len(results):,} rows)\n")
...
print(f"\n{'=' * 120}")
print("  HOLDOUT COMPARISON: best hyperparams with both objectives (7 features)")
print(f"  Params: {best}")
print("=" * 120)
...
ax4.set_title("Feature importances\n(reg:squarederror, best params, 7 features)",
              fontsize=11, fontweight="bold")
...
text = (
    f"HOLDOUT RESULTS (Sep 2024, 7 features)\n"
    f"{'─' * 44}\n\n"
...
fig.suptitle(
    "AAPL lit buy blocks — XGB GridSearchCV detail  (7 features, no day_of_week)\n"
    f"{n_combos:,} combos x 3-fold TimeSeriesSplit (Jun-Aug only)  |  "
    f"Best inner-CV MAE: {-gs.best_score_:.4f} bps",
    fontsize=13, fontweight="bold", y=1.02,
)

plt.savefig("aapl_gridsearch_detail_7feat.png", dpi=150, bbox_inches="tight")
print(f"\nSaved -> aapl_gridsearch_detail_7feat.png")
